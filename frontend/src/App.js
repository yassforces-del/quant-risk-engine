...
const RISK_ICONS = {
    low: lucide_react_1.CheckCircle,
    moderate: lucide_react_1.Activity,
    high: lucide_react_1.AlertTriangle,
...
function Sparkline({ prices, color }) {
    if (!prices || prices.length < 2)
        return <span style={{ color: '#2a4a2a', fontSize: 10 }}>NO DATA</span>;
    const w = 80, h = 28;
...
function App() {
    ...
    const [critical, setCritical] = (0, react_1.useState)(false);
    const [editingName, setEditingName] = (0, react_1.useState)(null);
    const [tempName, setTempName] = (0, react_1.useState)('');
    // autosave portfolios
    (0, react_1.useEffect)(() => {
    ...
    const setAssets = (updater) => {
        setPortfolios(prev => prev.map(p => p.id === activeId ? { ...p, assets: updater(p.assets) } : p));
    };
    ...
    const removeAsset = (i) => setAssets(p => p.filter((_, j) => j !== i));
    const updateAsset = (i, f, v) => setAssets(p => p.map((a, j) => j === i ? { ...a, [f]: v } : a));
    ...
    const confirmRename = () => {
        if (!editingName)
            return;
        setPortfolios(prev => prev.map(p => p.id === editingName ? { ...p, name: tempName.toUpperCase() || p.name } : p));
        setEditingName(null);
    };
    const analyze = async () => {
        setLoading(true);
        ...
        try {
            ...
            // save to portfolio
            setPortfolios(prev => prev.map(p => p.id === activeId ? { ...p, lastAnalysis: result } : p));
        }
        catch (e) {
            alert(e.message);
        }
    ...
    const RiskIcon = analysis ? (RISK_ICONS[analysis.riskLevel] ?? lucide_react_1.Activity) : lucide_react_1.Activity;
    ...
    return (<div style={styles.root}>
      ...
      <div style={styles.topBar}>
        ...
        <div style={styles.topRight}>
          <lucide_react_1.Clock size={12} color="#00ff88"/>
      ...
      <div style={styles.tabBar}>
        <div style={styles.tabList}>
          {portfolios.map(p => (<div key={p.id} style={{
                ...styles.tab,
                borderColor: activeId === p.id ? '#00ff88' : '#0a2a0a',
                color: activeId === p.id ? '#00ff88' : '#2a5a2a',
                background: activeId === p.id ? '#010f01' : 'transparent',
            }} onClick={() => setActiveId(p.id)}>
              {editingName === p.id ? (<input value={tempName} onChange={e => setTempName(e.target.value.toUpperCase())} onBlur={confirmRename} onKeyDown={e => e.key === 'Enter' && confirmRename()} autoFocus style={styles.tabInput} onClick={e => e.stopPropagation()}/>) : (<span onDoubleClick={e => { e.stopPropagation(); startRename(p); }}>
                  {p.name}
                </span>)}

              {/* score badge */}
              {p.lastAnalysis?.score !== undefined && (<span style={{
                    ...styles.tabScore,
              ...
              {portfolios.length > 1 && (<button onClick={e => { e.stopPropagation(); deletePortfolio(p.id); }} style={styles.tabClose}>
        ...
        <button onClick={createPortfolio} style={styles.newTabBtn}>
          <lucide_react_1.FolderPlus size={12}/> NEW FUND
        </button>
      </div>
      ...
      {isCritical && (<div style={{
          ...
          <lucide_react_1.XCircle size={14} color="#ff0033"/>
      ...
      <div style={styles.grid}>
        ...
        <div style={styles.panel}>
          <div style={styles.panelHeader}>
            <lucide_react_1.TrendingUp size={14} color="#00ff88"/>
          ...
          {assets.length === 0 && (<div style={styles.emptyAssets}>
              <span>NO POSITIONS — ADD AN ASSET</span>
            </div>)}

          {assets.map((a, i) => (<div key={i} style={styles.assetRow}>
              <input placeholder="BTC" value={a.symbol} onChange={e => updateAsset(i, 'symbol', e.target.value.toUpperCase())} style={{ ...styles.input, flex: 2, fontWeight: 700, color: '#00ff88', letterSpacing: 1 }}/>
              <input type="number" placeholder="0" value={a.quantity || ''} onChange={e => updateAsset(i, 'quantity', Number(e.target.value))} style={{ ...styles.input, flex: 2 }}/>
              <input type="number" placeholder="0" value={a.purchasePrice || ''} onChange={e => updateAsset(i, 'purchasePrice', Number(e.target.value))} style={{ ...styles.input, flex: 2 }}/>
          ...
          <button onClick={addAsset} style={styles.addBtn}>
            <lucide_react_1.Plus size={13}/> ADD POSITION
          </button>
        ...
        <div style={styles.panel}>
          <div style={styles.panelHeader}>
            <lucide_react_1.Activity size={14} color="#00ff88"/>
            <span>RISK ANALYSIS OUTPUT</span>
            <div style={{ marginLeft: 'auto', display: 'flex', gap: 8, alignItems: 'center' }}>
              {analysis?.source === 'cache' && (<span style={{ ...styles.badge, background: '#1a2a3a', color: '#60a5fa' }}>CACHED</span>)}
              {analysis && (<>
                  <button onClick={exportJSON} style={styles.exportBtn} title="Export JSON">
                    <lucide_react_1.Download size={11}/> JSON
                  </button>
                  <button onClick={exportPDF} style={styles.exportBtn} title="Export PDF">
                    <lucide_react_1.Download size={11}/> PDF
                  </button>
                </>)}
            </div>
          </div>

          {!analysis && !loading && (<div style={styles.empty}>
              <span style={styles.emptyText}>AWAITING ANALYSIS INPUT</span>
              <span style={styles.cursor}>_</span>
            </div>)}

          {loading && (<div style={styles.empty}>
              <div style={styles.loadingGrid}>
                {[...Array(12)].map((_, i) => (<div key={i} style={{ ...styles.loadingBlock, opacity: Math.random() > 0.5 ? 1 : 0.3 }}/>))}
              </div>
              <span style={styles.emptyText}>FETCHING MARKET DATA + AI ANALYSIS</span>
            </div>)}

          {analysis && (<div style={styles.resultBody}>
              <div style={{
                ...styles.riskBadge,
        ...
        // {analysis.riskType.toUpperCase()}</span>
              </div>

              {analysis.score !== undefined && (<div style={styles.scoreSection}>
              ...
              {analysis.stats && (<div style={styles.statsGrid}>
                  {[
                ...
                ].map(s => (<div key={s.label} style={styles.statBox}>
      ...
      {portfolios.filter(p => p.lastAnalysis).length > 1 && (<div style={styles.historyPanel}>
          <div style={styles.panelHeader}>
            <lucide_react_1.TrendingUp size={14} color="#00ff88"/>
          ...
          <div style={styles.historyGrid}>
            {portfolios.filter(p => p.lastAnalysis).map(p => (<div key={p.id} style={{
                    ...styles.historyItem,
                    borderColor: activeId === p.id ? '#00ff88' : '#0a2a0a',
                }} onClick={() => setActiveId(p.id)}>
      ...
      {/* HISTORY */}
      {history.length > 0 && (<div style={styles.historyPanel}>
          <div style={styles.panelHeader}>
            <lucide_react_1.Clock size={14} color="#00ff88"/>
            <span>ANALYSIS HISTORY</span>
            <span style={styles.badge}>{history.length} RECORDS</span>
          </div>
          <div style={styles.historyGrid}>
            {history.map((h, i) => (<div key={i} style={styles.historyItem} onClick={() => setAnalysis(h)}>
